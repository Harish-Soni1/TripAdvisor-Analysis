...
class PredictionFromModel:
    ...
    def predictData(self, file):
        ...
        try:
            ...
            polar_label = pd.Series(findPolarPrediction(prediction), name='label')
            data_with_label = pd.concat([rawData, polar_label], axis=1)
    ...
    def PredictRowData(self, data):
        ...
        try:
            ...
            model = fileOperation.loadModel()
            prediction = list(model.predict(data))
            polarLabel = pd.Series(findPolarPrediction(prediction), name='label')
            self.logger_object.log(self.file_object, 'Successfull End of Prediction !!!')
            return polarLabel
...
def findPolarPrediction(pred_list):
    polar = list()
    for i in pred_list:
        if i == 2:
